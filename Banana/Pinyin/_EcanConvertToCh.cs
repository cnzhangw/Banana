...
{
    [Obsolete]
    public static class _EcanConvertToCh
    {
        #region 数组信息
        ...
        /// 汉字转拼音
        /// </summary>
        /// <param name="cnInput"></param>
        /// <returns></returns>
        public static string ConvertToPinyin(string cnInput)
        {
            string pingyin = "";
            if (cnInput != "")
            {
                var strlist = cnInput.ToCharArray();

                if (Regex.IsMatch(strlist[0].ToString(CultureInfo.InvariantCulture), @"^[\u4E00-\u9FA5]+$"))
                {
                    for (int item = 0; item < strlist.Length; item++)
                    {
                        if (Regex.IsMatch(strlist[item].ToString(CultureInfo.InvariantCulture), @"^[\u4E00-\u9FA5]+$"))
                        {
                            string valuePY;
                            switch (item)
                            {
                                ...
                                case 0:
                                    valuePY = Get(strlist[item]);
                                    if (!string.IsNullOrEmpty(valuePY))
                                    {
                                        pingyin += valuePY;
                                    }
                                    else
                                    {
                                        pingyin += strlist[item];
                                    }
                                    break;
                                case 1:
                                    valuePY = Get(strlist[item]);

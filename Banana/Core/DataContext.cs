...
namespace Banana.Core
{
        ...
        where T : class, new()
    {
        ...
        public DataContext()
        {
            _db = DatabaseInitializer.Instance.Init();
        }
        ...
        /// <returns></returns>
        internal string[] GetColumns<U>(Database db, Expression<Func<U, object>> expression, bool inverse)
        {
            ...
            var columns_decrease = (from x in pd.Columns select x.Value.ColumnName).ToList();
        ...
        protected TResult Execute<TResult>(Func<Database, TResult> action)
        {
            TResult result = default(TResult);
            try
            {
                _db.OpenSharedConnection();
                result = action.Invoke(_db);
            }
            catch (Exception ex)
            {
        ...
        public U Single<U, Tid>(Tid id)
        {
            return Execute((db) =>
            {
                return db.Single<U>(primaryKey: id);
            });
        }
        ...
        public U Single<U>(Expression<Func<U, bool>> where)
        {
            return Execute((db) =>
            {
                string sql_parts = ExpressionHelper.AnalyticExpression(where);
                var sql = Sql.Builder.Append($"where {sql_parts}");
                return db.SingleOrDefault<U>(sql);
            });
        }
        ...
        public dynamic SingleDynamic(Sql sql)
        {
            return this.Execute((db) =>
            {
                ...
                return db.Connection.QuerySingleOrDefault(sql.ToString(), args);
            });
        }
        ...
        {
            return _db.Fetch<U>(page, limit, sql);
        }
        ...
        public bool Update<U>(U model, Expression<Func<U, object>> columns, bool inverse)
        {
            return Execute((db) =>
            {
                ...
                return db.Update(model, column_array) > 0;
            });
        }
        ...
        public bool Insert<U>(U model)
        {
            try
            {
                var pd = PocoData.ForType(typeof(U), _db.DefaultMapper);
                ...
                if (pk.GetValue(model) == null)
                {
                    ...
                    if (pk_type_name.Equals("string", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        PropertySet(model, new Dictionary<string, object>()
                        {
                            { propertyName , Guid.NewGuid().ToString("N") }
                        });
                    }
                }
                ...
                {
                    PocoColumn pc = null;
                    if (pd.Columns.TryGetValue(item, out pc))
                    {
        ...
        public Pager<T> Page(int page, int limit, Sql sql)
        {
            return this.Page<T>(page, limit, sql);
        }

        public Pager<U> Page<U>(int page, int limit, Sql sql)
        {
            return _db.Page<U>(page, limit, sql);
        }

        public Pager<dynamic> PageDynamic(int page, int limit, Sql sql)
        {
            return this.Execute((db) =>
            {
                ...
                var pageResult = new Pager<dynamic>();
        ...
        public Pager<DataTable> GetDataTable(int page, int limit, Sql sql)
        {
            return this.Execute((db) =>
            {
                var pageResult = new Pager<DataTable>()
                {
                    Page = page,
                    Limit = limit
                };
        ...
        public void ChangeDatabase(string key, Action callback)
        {
            string real_db_name = string.Empty;
            try
            {
                //get db name
                var dbs = Config.GetValue<JToken>("dbs");
                if (dbs != null)
                {
                    real_db_name = dbs.GetValue<string>(key);
                }

                if (real_db_name.IsNullOrWhiteSpace())
                {
                    throw new Exception($"未找到数据库 {key} 的配置");
                }
            }
            catch (Exception ex)
            {
            ...
            this.Execute<bool>((db) =>
            {
                ...
                {
                    db.Connection.ChangeDatabase(real_db_name);
                    callback.Invoke();
                    return true;
                }
                return false;

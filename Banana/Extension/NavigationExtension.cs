...
using static Banana.Dapper.SqlMapper;
    ...
    public static class NavigationExtension
    {
        ...
        public static T QueryFirstOrDefaults<T>(this IDbConnection dbCon, SqlProvider provider, IDbTransaction transaction)
        {
            if (provider.JoinList.Any(x => x.Action == JoinAction.Navigation && x.IsMapperField))
                ...
                return dbCon.QueryFirstOrDefault<T>(provider.SqlString, provider.Params, transaction, isExcludeUnitOfWork: provider.IsExcludeUnitOfWork);
        }
        ...
        public static Task<T> QueryFirstOrDefaultAsyncs<T>(this IDbConnection dbCon, SqlProvider provider, IDbTransaction transaction)
        {
            if (provider.JoinList.Any(x => x.Action == JoinAction.Navigation && x.IsMapperField))
                ...
                return dbCon.QueryFirstOrDefaultAsync<T>(provider.SqlString, provider.Params, transaction, isExcludeUnitOfWork: provider.IsExcludeUnitOfWork);
        }
        ...
        public static IEnumerable<T> Querys<T>(this IDbConnection dbCon, SqlProvider provider, IDbTransaction transaction)
        {
            if (provider.JoinList.Any(x => x.Action == JoinAction.Navigation && x.IsMapperField))
                ...
                return dbCon.Query<T>(provider.SqlString, provider.Params, transaction, isExcludeUnitOfWork: provider.IsExcludeUnitOfWork);
        }
        ...
        public static Task<IEnumerable<T>> QueryAsyncs<T>(this IDbConnection dbCon, SqlProvider provider, IDbTransaction transaction)
        {
            if (provider.JoinList.Any(x => x.Action == JoinAction.Navigation && x.IsMapperField))
                ...
                return dbCon.QueryAsync<T>(provider.SqlString, provider.Params, transaction, IsExcludeUnitOfWork: provider.IsExcludeUnitOfWork);
        }
        ...
        {
            return dbCon.QueryMultiple(provider.SqlString, provider.Params, transaction, isExcludeUnitOfWork: provider.IsExcludeUnitOfWork);
        }
        ...
        {
            return dbCon.QuerySingleOrDefault<T>(provider.SqlString, provider.Params, transaction, isExcludeUnitOfWork: provider.IsExcludeUnitOfWork);
        }
        ...
        {
            return await dbCon.QuerySingleOrDefaultAsync<T>(provider.SqlString, provider.Params, transaction, isExcludeUnitOfWork: provider.IsExcludeUnitOfWork);
        }
        ...
        public static DataSet QueryDataSets(this IDbConnection dbCon, SqlProvider provider, IDbTransaction transaction, IDbDataAdapter dataAdapter)
        {
            ...
            return dbCon.QueryDataSet(dataAdapter, provider.SqlString, provider.Params, transaction, isExcludeUnitOfWork: provider.IsExcludeUnitOfWork);
        }
        ...
        /// <param name="dataAdapter"></param>
        /// <returns></returns>
        public static async Task<DataSet> QueryDataSetsAsync(this IDbConnection dbCon, SqlProvider provider, IDbTransaction transaction,
            IDbDataAdapter dataAdapter)
        {
            if (dataAdapter == null)
                dataAdapter = new SqlDataAdapter();
            return await dbCon.QueryDataSetAsync(dataAdapter, provider.SqlString, provider.Params, transaction, isExcludeUnitOfWork: provider.IsExcludeUnitOfWork);
        }
    }
}

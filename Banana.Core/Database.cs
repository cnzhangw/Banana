    ...
    public class Database : IDatabase
    {
        ...
        private void AddParam(IDbCommand cmd, object value, PropertyInfo pi)
        {
            ...
            else
            {
                ...
                {
                    if (Provider is Providers.SQLiteDatabaseProvider) // fixed for sqlite on 2019-1-8 16:31:23
                    {
                        // https://www.cnblogs.com/bigben0123/p/4212896.html
                        p.Value = Convert.ToDateTime(value).ToString("yyyy-MM-dd HH:mm:ss");
                    }
                    else
                    {
                        p.Value = value;
        ...
        internal void BuildPageQueries<T>(long skip, long take, string sql, ref object[] args, out string sqlCount, out string sqlPage)
        {
        ...
        internal void BuildPageQueries(int skip, int take, string sql, ref object[] args, out string sqlCount, out string sqlPage)
        {
            //非泛型，无需对类型进行拼装查询字段 by zhangw 2018-6-14
        ...
        ///     object.
        /// </remarks>
        public PagingResult<T> Page<T>(long page, long itemsPerPage, string sqlCount, object[] countArgs, string sqlPage, object[] pageArgs)
        {
            ...
            var result = new PagingResult<T>
            {
                Page = page,
                Limit = itemsPerPage,
                Count = ExecuteScalar<long>(sqlCount, countArgs)
            };
            result.PageCount = result.Count / itemsPerPage;
            ...
            result.Data = Fetch<T>(sqlPage, pageArgs);
        ...
        /// </remarks>
        public PagingResult<T> Page<T>(long page, long itemsPerPage, string sql, params object[] args)
        {
        ...
        ///     total number of records in the result set.
        /// </remarks>
        public PagingResult<T> Page<T>(long page, long itemsPerPage, Sql sql)
        {
            return Page<T>(page, itemsPerPage, sql.SQL.TrimEnd(';'), sql.Arguments);
        }
        ...
        ///     object.
        /// </remarks>
        public PagingResult<T> Page<T>(long page, long itemsPerPage, Sql sqlCount, Sql sqlPage)
        {
        ...
        public int Update(string tableName, string primaryKeyName, object poco, IEnumerable<string> columns, string where)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        cmd.CommandText = string.Format("UPDATE {0} SET {1} WHERE {2}",
                            _provider.EscapeTableName(tableName), sb.ToString(), where);

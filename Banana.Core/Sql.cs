...
namespace Banana.Core
{
    ...
    public class Sql
    {
        ...
        public Sql Append(ColumnPairs pairs)
        {
            List<string> values = new List<string>();
            foreach (var item in pairs)
            {
                values.Add($"{item.Key} as {item.Value}");
            }
            return Append((pairs.PrependComma ? ", " : "") + string.Join(", ", values));
        }
        ...
        {
            return Append(new Sql("ORDER BY " + string.Join(", ", (from x in columns where x.ToString().HasValue() select x.ToString()).ToArray())));
        }
        ...
        {
            return Append(new Sql($"FROM {table} AS {alias}"));
        }
        ...
        {
            return Join("LEFT JOIN ", table + " AS " + alias);
        }
        ...
        /// 多字段查询对儿
        /// </summary>
        public class ColumnPairs : Dictionary<string, string>
        {
            internal bool PrependComma { get; set; }

            /// <summary>
            /// 创建sql多字段查询对儿
            /// </summary>
            /// <param name="comma">前面追加逗号</param>
            public ColumnPairs(bool comma = false)
            {
                this.PrependComma = comma;
            }

            /// <summary>
            /// 创建sql多字段查询对儿
            /// </summary>
            /// <param name="key">数据列，如：usr.name_pinyin</param>
            /// <param name="value">列别名，如：NamePinyin</param>
            /// <param name="comma">前面追加逗号</param>
            public ColumnPairs(string key, string value, bool comma = false)
            {
                this.PrependComma = comma;
                this.Add(key, value);
            }
            ...
            /// </summary>
            /// <returns></returns>
            public ColumnPairs CommaPrepend()
            {
                PrependComma = true;
                return this;
            }
